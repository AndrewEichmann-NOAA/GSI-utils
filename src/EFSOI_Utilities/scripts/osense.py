...
def read_osense(filename):
    """read osense file generated by enkf and return two DataFrames with 
    conventional/ozone data and satellite data, and the cycle date read"""
...
# this causes more trouble than it solves, but might be useful later
    ...
    osensedata = pd.concat([satdata[osensefields],
                            convbycodes[osensefields]], ignore_index=True)
    return osensedata


def make_cycles(firstcycle, lastcycle):
    """generate pandas date range from first and last cycles as datetimes"""
...
def add_channels(osensedata, satinfofile):
    """take osense DataFrame and GSI satinfo file and add column with satellite
    channels"""

    # get satinfo and make index start at 1 like indxsat
    satinfo = pd.read_csv(satinfofile, header=0, delim_whitespace=True)
    satinfo.index += 1
    # convert chan to Int64 from int64 so that the int will be preserved in
    # the merge
    satinfo.chan = satinfo.chan.convert_dtypes()

    # The following is to make sure the satinfo is consistent with the osense
    # data

    # everything with a channel has indxsat > 0
    satindexes = osensedata.loc[osensedata['indxsat']
                                > 0, ['indxsat', 'detailed_source']]
    # get the satinfo line for every obs
    satinfoperobs = satinfo.loc[satindexes['indxsat'], [
        '!sensor/instr/sat', 'iuse']]

    if -1 in satinfoperobs.iuse.unique():
        print("WARNING: Supposedly unused channel detected in osense.")
        print("Are you using the right satinfo file?")
        sys.exit(1)

    sourcematch = satindexes['detailed_source'].values == \
        satinfoperobs['!sensor/instr/sat'].values
    if not sourcematch.all():
        print("WARNING: Observation sources not matching satinfo sensors.")
        print("Are you using the right satinfo file?")
        sys.exit(1)

    # looks good, so merge
    osensedata = osensedata.merge(
        satinfo['chan'],
        how='left',
        left_on='indxsat',
        right_index=True,
